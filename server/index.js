...
import { buildNotesContext, promptSafeName, notebookMemberNames, aiErrorDetail, validateFeynmanResult, anthropicClient, getModel } from "./lib/ai.js";
import { requireAuth, requireMember } from "./lib/auth.js";
import { aiLimiter, checkoutLimiter, explainLimiter, feynmanLimiter, forgeLimiter, globalLimiter, otpIpLimiter, otpSendEmailLimiter, otpVerifyLimiter, podcastLimiter, queryLimiter, resetLimiter, webhookLimiter } from "./lib/limiters.js";
...
import { FREE_IMAGE_LIMIT, FREE_MSG_LIMIT, FREE_MSG_WARN, checkClassLimit, checkNotebookLimit, checkUsageLimit, countOwnedNotebooks, getUserIdByStripeCustomer, getUserTier, incrementUsage, recordProCost, resetUsageIfNeeded } from "./lib/usage.js";
...
app.get("/api/notebooks/:id/flashcards", requireAuth, requireMember, async (req, res) => {
  const { data, error } = await supabase
    .from("flashcards")
    .select("id, notebook_id, front, back, ease_factor, interval_days, repetitions, due_date, last_reviewed, created_at")
    .eq("notebook_id", req.params.id)
...
app.post("/api/squad/join/:token", requireAuth, async (req, res) => {
  const { data: squad } = await supabase
    .from("squads")
    .select("id, seats")
